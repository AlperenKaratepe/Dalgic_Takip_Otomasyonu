...
namespace dalisapp
{
 ...
 con.Open();
        ...
        {
            con.Open();
            string kayit = "update db set dalis=@dalis,ekipman=@ekipman,su=@su,derinlik=@derinlik,süre=@süre,sicaklik=@sicaklik,partner=@partner,kaptan=@kaptan,tarih=@tarih,aciklama=@aciklama where adsoyad=@adsoyad";
            SQLiteCommand cmd = new SQLiteCommand(kayit, con);
